...
func TestSync(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			admissionCheckInformer.Informer().AddIndexers(cache.Indexers{
			...
			for _, obj := range c.clusterObjects {
				...
				case *clusterv1beta1.Placement:
					placementInformer.Informer().GetStore().Add(o)
				case *clusterv1beta1.PlacementDecision:
					placementDecisionInformer.Informer().GetStore().Add(o)
				}
			}
			for _, obj := range c.kueueObjects {
				if ac, ok := obj.(*kueuev1beta1.AdmissionCheck); ok {
					admissionCheckInformer.Informer().GetStore().Add(ac)
				}
			}
